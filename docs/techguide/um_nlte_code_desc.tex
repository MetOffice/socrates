...
Here,~\textit{x} is a dimensionless vertical scale height.
...
\item  $12.5 \leq x < 14$ \\ 
   CO$_2$ volume mixing ratio proportional to scale height \textit{x}. \\
...
\subsection{Set up vertical scale height (\textit{x}) and corresponding pressure levels}

   $x = \ln \frac{100000}{p_x}$

   \paragraph{Outputs}
   \begin{enumerate}
   \item \textit{x}: Dimensionless scale height, set in code from 
     $0 \leq x \leq 17.5$ with steps of $\Delta x = 0.25$ (bottom of atmosphere 
     to top)
   \item $p_x$: Pressure corresponding to the x-levels \textit{x} (Pa)
   \end{enumerate}
...
\subsection{Interpolate the UM temperatures and gas mix ratios to \textit{x}}
   ...
   \paragraph{Outputs}
   \begin{enumerate}
   \item $T_x$: Temperatures corresponding to the dimensionless scale \textit{x}, 
     (bottom of atmosphere to top) (K)
   \item $g_{x,q}$: Mass mixing ratios for different gases q corresponding 
     to the dimensionless scale \textit{x} (bottom of atmosphere to top) (ppm)
   \end{enumerate}
...
\subsection{Convert mass mixing ratios to volume mixing ratios over scale required}
   ...
   \item $c_{x,q}$: Volume mixing ratios for different gases q corresponding 
     to the dimensionless scale \textit{x} (parts per million volume, ppmv)
   \end{enumerate}
...
\subsection{Calculate the exponential part of the Planck function for CO2 and O3}
   ...
   \item $T_x$: Temperature per scale height \textit{x} (K)
   \end{enumerate}
...
\subsection{Calculate the CO2 coefficients for the matrix parameterisation 2 \textless \textit{x} \textless 12.5 for the particular concentration of CO2}

   \begin{itemize}
   \item For each given scale height \textit{x}, height level $j$ and concentration 
   ...
   \item Fomichev 98, section 3.1 paragraph 3 suggests 360ppm is used 
     ...
     \small{\url{http://ds.data.jma.go.jp/gmd/wdcgg/pub/products/summary/sum41/10_03co2.pdf}}\\
...
\subsection{Calculate the CO2 column amount for 12.5 \textless \textit{x} \textless 16.5}

   The integral over the depth of the atmosphere defines the \emph{atmospheric column} of CO$_2$ (or another gas species~\textit{x}) as
   %
   \[
   u_{x, \text{CO}_2}\ =\ \int n_{x, \text{CO}_2} dx,
   \]

   \noindent where~\textit{x} is the dimensionless scale height and~$n_{x, \text{CO}_2}$ is the number density of CO$_2$ (molecules cm$^{-3}$). The number density~$n_{x, \text{CO}_2}$ and the volume mixing ratio~$c_{x, \text{CO}_2}$ are related by the number density of air~$n_{x, a}$ as follows:
   %
   \[
   ...
   Next, differentiate this expression with respect to the dimensionless scale height~\textit{x}. Note that~${p_x = 10^5 e^{-x}}$, and so~${dp = -10^5 e^{-x}}$. Then:
%
   \begin{alignat}{4}
...
%
   ...
   \item \textit{x}: Dimensionless scale height (dimensionless)
...
\subsection{Calculate the coefficients dj for the recurrence formula for 12.5 \textless \textit{x} \textless 16.5}
   ...
   \begin{itemize}   
   \item $L_u$ for \textit{x} is found by linear interpolation of 
     log $L_u$ over $u_x$.
   \item $\alpha_x$ for \textit{x} is found by linear interpolation of 
     log $\alpha_{u,x}$ over $u_x$, where a different log $\alpha_{u,x}-u_x$
     association is available for each \textit{x}.
   ...
   \item $\alpha_{u,x}$: Correction to escape function for CO$_2$ column 
     amount $u_x$ and scale height \textit{x} 
   \end{enumerate}
   ...
   \item Fomichev 98 table 11 - Correction to escape function $\alpha_{u,x}$ 
     for CO$_2$ column amount $u_x$ and for \textit{x} 
...
\subsection{Calculate number density}
   ...
   \item $T_x$: Temperature at scale height \textit{x} (K)
   \end{enumerate}

   \paragraph{Outputs}
   \begin{enumerate}
   \item $\rho_x$: Number density at scale height \textit{x} (m$^3$)
   \end{enumerate}

\subsection{Calculate parameter lambda for use in the recurrence formula}
...
\subsection{Calculation of the boundary condition at \textit{x}=12.5 for the recurrence formula}
...
\subsection{Solve the recurrence formula for the heating rate correction, and calculate the heating rate for 12.5 \textless \textit{x} \textless 16.5}
...
\subsection{Calculation of the heating rate above \textit{x}=16.5}
...
\subsection{Convert heating rates into K/day using the specific heat capacity} 
...
\subsection{Heating rates interpolated to the original pressure levels}
   ...
   \item $\epsilon_x$: Heating rates corresponding to the dimensionless 
     scale height \textit{x} (J kg$^{-1}$ s$^{-1}$)
   \item $p_x$: Pressure corresponding to the x-levels \textit{x} (converted
...
\small{\url{https://www.cfa.harvard.edu/atmosphere/publications/2006-EinsteinA-JQSRT-98.pdf}}
...
\subsection{Scale CO2 column amount by the tropospheric CO2 volume mixing ratio}
...
\subsection{Calculate the amount of CO2 along the zenith path}
